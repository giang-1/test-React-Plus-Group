...
function App() {
  ...
  return (
    <div className="container ">
      <div>
        {
          pointOfChracterA > pointOfChracterB ? <h4>A is Winner</h4> : pointOfChracterA === pointOfChracterB ? <h4>same point</h4> : <h4>B is Winner</h4>
        }
      </div>
      <div>
        <p>character A</p>
        {new Array(pointOfChracterA).fill(1).map((point, index) => (
          <button className='btn btn-primary' key={index}></button>
        ))}
        <p>character B</p>
      ...
      <div>
        <button className='btn btn-sm btn-success ' onClick={ramdomPlusPointForCharacter}>race</button>
